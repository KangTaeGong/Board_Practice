...
public class BoardDAO {
	...
	public void insertBoard(BoardBean bean) {
		...
		int ref=0;	// 글 그룹을 의미 - 쿼리를 실행시켜서 가장큰 ref값을 가져온 후 +1
		...
		try {
			...
			if(rs.next()) {
				ref = rs.getInt(1) + 1;	// 최대값에 +1을 해서 글 그룹을 설정
			}
...
//			String sql = "select * from board order by ref desc, re_step asc, re_level asc";
			...
			// 쿼리 실행 후 결과 저장
			rs = pstmt.executeQuery();
			// 데이터 갯수가 몇개인지 모르기에 반복문을 이용하여 데이터를 추출
			while(rs.next()) {
				// 데이터를 패키징(가방 = BoardBean.class)
	...
	public void reWriteBoard(BoardBean bean) {
		...
		try {
			...
			pstmt.setInt(6, re_step+1);	// 답글이기에 부모 글 re_step에 +1
	...
	// BoardUpdate용 하나의 게시글 리턴(Delete용도 가능)
	...
	// Update, Delete시 필요한 패스워드 값을 리턴해주는 메소드
